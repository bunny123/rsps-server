...
import server.model.players.FamiliarSpecials;
import server.model.players.Curses.*;
...
if(Server.npcHandler.npcs[i].index != c.playerId || c.wildLevel <= 1) {
			...
			if(c.attackTimer <= 0) {
			 ...
			 if (Misc.random(22) == 0) {
						c.getEffects().npcleechAttack(0);
                }
                if (Misc.random(23) == 0) {
                        c.getEffects().npcleechDefence(0);
                }
                if (Misc.random(24) == 0) {
                        c.getEffects().npcleechStrength(0);
                }
                if (Misc.random(25) == 0) {
                        c.getEffects().npcleechSpecial(0);
                }
				if (Misc.random(20) == 0) {
                        c.getEffects().npcSapWarrior(0);
                }
				if (Misc.random(21) == 0) {
                        c.getEffects().npcSapRange(0);
                }
				if (Misc.random(23) == 0) {
                        c.getEffects().npcSapMagic(0);
                }
				if (Misc.random(20) == 0) {
                        c.getEffects().npcSapWarrior(0);
                }
                if (Misc.random(26) == 0) {
                        c.getEffects().npcleechRanged(0);
                }
                if (Misc.random(27) == 0) {
                        c.getEffects().npcleechMagic(0);
                }
	...
	**/
		...
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.attackTimer <= 0) {
				...
				c.getEffects().applyLeeches(i);
				c.getEffects().applySaps(i);
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				c.getDeflects().handleRangeDeflects(c, i, damage);
				...
				c.getDeflects().handleMageDeflects(c, i, damage);
	...
	public void applyPlayerMeleeDamage(int i, int damageMask, int damage){
		...
		c.getDeflects().handleMeleeDeflects(c, i, damage);
	...
	/**
	* Methods we use for handling switching 
	*/
	public void strCurse(int i) {
		for (int j = 0; j < str.length; j++) {
			if (str[j] != i) {
				c.prayerActive[str[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[str[j]], 0);
			}								
		}
	}
	public void atkCurse(int i) {
		for (int j = 0; j < atk.length; j++) {
			if (atk[j] != i) {
				c.prayerActive[atk[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[atk[j]], 0);
			}						
		}
	}
	public void defCurse(int i) {
		for (int j = 0; j < def.length; j++) {
			if (def[j] != i) {
				c.prayerActive[def[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[def[j]], 0);
			}								
		}
	}
	public void rngCurse(int i) {
		for (int j = 0; j < rng.length; j++) {
			if (rng[j] != i) {
				c.prayerActive[rng[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[rng[j]], 0);
			}								
		}
	}
	public void mgeCurse(int i) {
		for (int j = 0; j < mge.length; j++) {
			if (mge[j] != i) {
				c.prayerActive[mge[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[mge[j]], 0);
			}								
		}
	}
	public void sprtCurse(int i) {
		for (int j = 0; j < sprt.length; j++) {
			if (sprt[j] != i) {
				c.prayerActive[sprt[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[sprt[j]], 0);
			}								
		}
	}
	public void headIcoCurse(int i) {
		for (int j = 0; j < hedico.length; j++) {
			if (hedico[j] != i) {
				c.prayerActive[hedico[j]] = false;
				c.getPA().sendFrame36(c.PRAYER_GLOW[hedico[j]], 0);
			}								
		}
	}
		...
		public int[] def = {39, 45};
		public int[] str = {40, 45};
		public int[] atk = {27, 36, 45};
		public int[] rng = {28, 37, 45};
		public int[] mge = {29, 38, 45};
		public int[] sprt = {30, 42};
		public int[] hedico = {43, 44, 45};
		
	public void activatePrayer(int i) {
		if (i > 25 && c.getPA().getLevelForXP(c.playerXP[1]) < 28) {
			c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
			c.sendMessage("You need 28 defence to use curses.");
			return;
		}
		if(c.duelRule[7]){
	...
	if(c.prayerActive[10] == false) {
				...
				break;	
					...
					case 39: //leach defence
					if (c.prayerActive[i] == false) {
							defCurse(i); 
						}
					break;
					
					case 40: //leech strength
					if (c.prayerActive[i] == false) {
							strCurse(i);
						}
					break;
					...
					if(System.currentTimeMillis() - c.stopPrayerDelay < 5000) {
							c.sendMessage("You have been injured and can't use this prayer!");
							c.getPA().sendFrame36(c.PRAYER_GLOW[33], 0);
							c.getPA().sendFrame36(c.PRAYER_GLOW[34], 0);
							c.getPA().sendFrame36(c.PRAYER_GLOW[35], 0);
							return;
						}
						if (i == 33)
							c.protMageDelay = System.currentTimeMillis();
					...
					case 44://soulsplit
						...
						headIcoCurse(i);				
						for(int p = 32; p < 45; p++) {
							if(i != p && p != 36 && p != 37 && p != 45 && p != 38 && p != 39 && p != 40 && p != 41 && p != 42) {
								c.prayerActive[p] = false;
								c.getPA().sendFrame36(c.PRAYER_GLOW[p], 0);
							}
						}
...
break;
		...
		c.specDamage = 2.25;
		c.doingKorasiSpec = true;
		c.gfx100(1224);
        EventManager.getSingleton().addEvent(new Event() {
		int KTime = 2;
	public void execute(EventContainer korasi) {
	...
	if (KTime == 1) {
		...
		o.gfx100(1224);
		applyPlayerMageDamage((int) (Misc.random(calculateMeleeMaxHit())));
		}
	}
		if (KTime < 1) {
			korasi.stop();
				c.doingKorasiSpec = false;
     	         return; 
	}
		if (KTime > 0) {
			KTime--;
	...
	**/
	...
	public int getRangeStr(int i) {
		...
		int[][] data = {
			...
			{11212, 60}, {15243, 60}, {806, 1}, {807, 3}, {808, 4}, 
	...
	public void handleDfs() {
		if (System.currentTimeMillis() - c.dfsDelay > 30000) {
			if (c.playerIndex > 0 && Server.playerHandler.players[c.playerIndex] != null) {
				...
				Server.playerHandler.players[c.oldPlayerIndex].hitDiff2 = new Hit(damage, CombatType.DEFLECT);
	...
	public void handleZerker() {
		...
		if (System.currentTimeMillis() - c.dfsDelay > 60000) {
			if (c.playerIndex > 0 && Server.playerHandler.players[c.playerIndex] != null) {
				...
				c.gfx0(369);
				Server.playerHandler.players[c.playerIndex].playerLevel[3] -= damage;
				Server.playerHandler.players[c.playerIndex].hitDiff2 = new Hit(damage, CombatType.DEFLECT);
	...
	public void handleArcher() {
		...
		public void handleDfsNPC() {
			...
			if (c.npcIndex > 0 && Server.npcHandler.npcs[c.npcIndex] != null) {
				...
				Server.npcHandler.npcs[c.npcIndex].hitDiff2 = new Hit(damage, CombatType.DEFLECT);
		...
		public void applyDeflect(int damage, int i) {
		if (damage > 0 && Server.playerHandler.players[i].playerEquipment[c.playerRing] == 2550) {
			int recDamage = damage/10 + 1;
			if (!c.getHitUpdateRequired()) {
				c.hitDiff = new Hit(recDamage, CombatType.DEFLECT);
