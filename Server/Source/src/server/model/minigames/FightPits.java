...
public class FightPits {
		...
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				...
				if (c.getPA().inPitsWait() || c.inPits) {
					...
					c.getPA().sendQuest("Champion: "+pitsChampion, 15897);
					c.getPA().sendQuest("Foes Remaining: "+playersRemaining, 15898);
	...
	public void startGame() {
		if (getWaitAmount() < 2) {
			...
			//sendMessage("Unable to start fight pits game due to lack of players.");
			return;
		}	
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null )  {
