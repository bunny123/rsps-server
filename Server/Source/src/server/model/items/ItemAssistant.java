...
Server.itemHandler.createGroundItem(o, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName.endsWith("bow") || WeaponName.endsWith("10")|| c.playerEquipment[c.playerWeapon] == 15241 || WeaponName.endsWith("full") || WeaponName.startsWith("seercull")) {
	...
	**/
	public boolean is2handed(String itemName, int itemId) {
		if(itemName.contains("ahrim") || itemName.contains("karil") || itemName.contains("verac") || itemName.contains("guthan") || itemName.contains("dharok") || itemName.contains("torag") || itemName.contains("maul")) {
			return true;
		}
			...
			if(c.playerItems[slot] == (wearID+1)) {				
				...
				targetSlot = Item.targetSlots[wearID];
					if(itemType(wearID).equalsIgnoreCase("cape")) {
						targetSlot = 1;
					} else if(itemType(wearID).equalsIgnoreCase("hat") || itemType(wearID).endsWith("hood")) {
						targetSlot = 0;
	...
	**/
	...
	public void createGroundItem(int itemID, int itemX, int itemY,
		...
		//synchronized (c) {
			...
			c.getOutStream().writeByteC((itemY - 8 * c.getMapRegionY()));
			c.getOutStream().writeByteC((itemX - 8 * c.getMapRegionX()));
