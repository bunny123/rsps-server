			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.specTimer = Integer.parseInt(token2);
                       ...
                       p.snowOn = Integer.parseInt(token2);
						...
						for (int j = 0; j < token3.length; j++) {
							p.quickCurses[j] = Boolean.parseBoolean(token3[j]);						
						}
					} else if (token.equals("QuickPrayers")) {
						for (int j = 0; j < token3.length; j++) {
							p.quickPrayers[j] = Boolean.parseBoolean(token3[j]);						
						}
	...
	**/
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("specTimer = ", 0, 11);
			characterfile.write(Integer.toString(p.specTimer), 0, Integer.toString(p.specTimer).length());
			characterfile.newLine();
			...
			String toWrite3 = "";
			for(int i1 = 0; i1 < p.quickCurses.length; i1++) {
				toWrite3 += p.quickCurses[i1] +"\t";
			}
			characterfile.write(toWrite3);
			characterfile.newLine();
			characterfile.write("QuickPrayers = ", 0, 15);
			String toWrite4 = "";
			for(int i1 = 0; i1 < p.quickPrayers.length; i1++) {
				toWrite4 += p.quickPrayers[i1] +"\t";
			}
