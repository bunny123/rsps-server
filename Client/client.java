...
public static void setTab(int id) {
	...
	public Sprite[] ORB = new Sprite[20];
	...
	public byte[] GetMap(int Index) {
		try {
			File Map = new File("./Cache/Maps/"+Index+".gz");
	...
	public void switchGameframe(int version) {
		if(version == 508) {
			is508 = false;
			is474 = true;
			chatArea = new Sprite("Custom/508/chatarea");
			tabArea = new Sprite("Custom/508/tabarea");
			mapArea = new Sprite("Custom/508/maparea");
			chatButtons = new Sprite("Custom/508/chatbuttons");
			logIcon = new Sprite("Custom/508/logIcon");
			needDrawTabArea = true;
			aBoolean1233 = true;
			newMap = false;
		} else 
		if(version == 474) {
			is474 = true;
			is508 = false;
			chatArea = new Sprite("Custom/508/chat");
			tabArea = new Sprite("Custom/508/tab");
			mapArea = new Sprite("Custom/508/map");
			chatButtons = new Sprite("Custom/508/chatb");
			logIcon = new Sprite("Custom/508/logout");
			needDrawTabArea = true;
			aBoolean1233 = true;
			newMap = true;
		}
	...
	public void drawChannelButtons() {
		...
		if(newMap) {
		/* Draws main buttons sprite */
		chatButtons.drawSprite(5, 142);
		/* Button hovering */
		if(cButtonHPos == 7) {
			reportH.drawSprite(404, 142);
		}
		if(cButtonCPos == 0)
				chatButtonC.drawSprite(5, 142);
			else if(cButtonCPos == 1)
				chatButtonC.drawSprite(71, 142);
			else if(cButtonCPos == 2)
				chatButtonC.drawSprite(137, 142);
			else if(cButtonCPos == 3)
				chatButtonC.drawSprite(203, 142);
			else if(cButtonCPos == 4)
				chatButtonC.drawSprite(269, 142);
			else if(cButtonCPos == 5)
				chatButtonC.drawSprite(335, 142);
		if(cButtonHPos == cButtonCPos) {
			if(cButtonHPos == 0)
				chatButtonHC.drawSprite(5, 140);
			else if(cButtonHPos == 1)
				chatButtonHC.drawSprite(71, 140);
			else if(cButtonHPos == 2)
				chatButtonHC.drawSprite(137, 140);
			else if(cButtonHPos == 3)
				chatButtonHC.drawSprite(203, 142);
			else if(cButtonHPos == 4)
				chatButtonHC.drawSprite(269, 142);
			else if(cButtonHPos == 5)
				chatButtonHC.drawSprite(335, 142);
		} else {
			if(cButtonHPos == 0)
				chatButtonH.drawSprite(5, 142);
			else if(cButtonHPos == 1)
				chatButtonH.drawSprite(71, 142);
			else if(cButtonHPos == 2)
				chatButtonH.drawSprite(137, 142);
			else if(cButtonHPos == 3)
				chatButtonH.drawSprite(203, 142);
			else if(cButtonHPos == 4)
				chatButtonH.drawSprite(269, 142);
			else if(cButtonHPos == 5)
				chatButtonH.drawSprite(335, 142);
		}
		/* Button text */
	 ...
	 private void drawChatArea() {
        ...
        if(messagePromptRaised) {
           ...
           //newRegularFont.drawBasicString(inputString + ((loopCycle % 40 < 20) ? "<col=#000000>|" : ""), 12 + textDrawingArea.getTextWidth(s + ": "),133+y,255,-1);
		    newRegularFont.drawBasicString(inputString + ((loopCycle % 40 < 20) ? "<col=#000000>|" : ""), 12 + textDrawingArea.getTextWidth(s + ": "),133,255,-1);
...
int draw, fill, col;
/*if(Config.is474) {
draw = 0xFFFFA0; fill = 0; col = 0;
} else {*/
draw = 0x0E0E0E; fill = 0xEBECE6; col = 0xF5B241;
//}
DrawingArea.drawPixels(height, yPos, xPos, draw, width);
DrawingArea.fillPixels(xPos, width, height, fill, yPos);
yPos += 14;
for(int i = 0; i < results.length; i++) {
aTextDrawingArea_1271.method389(false, xPos + 3, col, results[i], yPos);
yPos += 16;
}
}
}
						...
						if (spriteDrawX > -1) {
							...
							if (!HPBarToggle == true) {
								...
								HPBarFull = new Sprite(sign.signlink
										.findcachedir()
										+ "Sprites/newSprites/HITPOINTS_0.PNG", HpPercent,
										7);
	...
	public void drawSideIcons(){
		...
		if(tabInterfaceIDs[16] != -1)
            newSideIcons[15].drawSprite(8, 307);
    }
...
if (menuToggle == true) {
  ...
  //Menu Fill
  //DrawingArea.method335(fill, yPos + 20, menuW - 6, menuH - 23, 280, xPos + 3);
...
//		hasFollower = false;
	...
	void drawLoadingText(int i, String s)
    {
		...
		if(titleStreamLoader == null) {
        	...
        	aRSImageProducer_1109.initDrawingArea();		
			loadingBarEmpty = new Sprite("Interfaces/Configuration/empty");
			loadingBarFull = new Sprite("Interfaces/Configuration/full");
			char c = '\u0168';
			char c1 = '\310';
			byte byte1 = 20;
			int j = c1 / 2 - 18 - byte1;
  ...
  */
	...
	private void doAction(int i)
	{
		...
		if(l == 1501) {
				if(!restOrb) {
					restOrb = true;
					stream.createFrame(185); 
					stream.writeWord(6602);
				} else {
					restOrb = false;
					stream.createFrame(185); 
					stream.writeWord(6603);
				}
			}
	...
	if(l == 1005) {
		...
		if (l == 1500) {
				if(!prayClicked){
					prayClicked = true;
					stream.createFrame(185); 
					stream.writeWord(6600);
				} else {
					prayClicked = false;
					stream.createFrame(185); 
					stream.writeWord(6601);
				}
				}
		if(l == 1004) {
		...
		do {
			...
			if(openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				if((j == 13 || j == 10) && inputString.length() > 0) {
					if(myPrivilege == 2 || server.equals("acelta.ddns.net") || 1 == 1/*to remove*/) {
	...
	private void processMinimapActions() {
		...
		if(super.mouseX >= 743 && super.mouseX <= 765 && super.mouseY >= 0 && super.mouseY <= 21) {
menuActionName[1] = "Exit";
menuActionID[1] = 100;
menuActionRow = 2;
	...
	/*} else if (super.mouseX >= 687 && super.mouseX <= 744 && super.mouseY >= 130 && super.mouseY < 157) {
		...
		if(super.mouseX >= 707 && super.mouseX <= 762 && super.mouseY >= 92 && super.mouseY < 125){
			if(!runClicked){
				menuActionName[2] = "Turn run mode on";
			} else if(runClicked){
				menuActionName[2] = "Turn run mode off";
			}
			menuActionID[2] = 1050;
			menuActionRow = 2;
			...
			if(super.mouseX >= 706 && super.mouseX <= 762 && super.mouseY >= 52 && super.mouseY < 87){
				menuActionName[2] = prayClicked ? "Turn Prayer off" : "Turn quick prayers on";
				menuActionID[2] = 1500;
				menuActionRow = 2;
			...
			/*if(super.mouseX >= 706 && super.mouseX <= 762 && super.mouseY >= 52 && super.mouseY < 87){
				if(!prayClicked){
					menuActionName[2] = "Turn quick prayers on";
				} else if(prayClicked){
					menuActionName[2] = "Turn quick prayers off";
				}
				menuActionID[2] = 1500;
				menuActionRow = 2;
				menuActionName[1] = "Select quick prayers";
	...
	public void processRightClick() {
	   ...
	   if(super.mouseX > 548 && super.mouseY > 207 && super.mouseX < 740 && super.mouseY < 468) {
		...
		if (super.mouseX >= 515 && super.mouseY >= 0 && super.mouseX <= 765 && super.mouseY <= 167)
            if (openInterfaceID != -1)
		...
		if(signlink.cache_dat != null)
		{
           ...
           titleStreamLoader = streamLoaderForName(1, "title screen", "title", expectedCRCs[1], 25);
			...
			for(int bh = 2; bh < 7; bh++) {
				bountyHunter[bh] = new Sprite("Custom/Bounty/bounty "+bh+"");
			}
		
			/*loginBox = new Sprite("Custom/Gameframe/title");
			loginBoxHover = new Sprite("Custom/Gameframe/box_hover");
			loginHover = new Sprite("Custom/Gameframe/login_hover");
			loginHoverWorld = new Sprite("Custom/Gameframe/login_hover2");*/
			chatButtonH = new Sprite("Custom/508/chathover");
			chatButtonC = new Sprite("Custom/508/chatclicked");
			chatButtonHC = new Sprite("Custom/508/chatclickedh");
			reportH = new Sprite("Custom/508/reporthover");
			logIconH = new Sprite("Custom/508/logiconh");
			logIconC = new Sprite("Custom/508/logiconc");
			emptyOrb = new Sprite("Custom/508/emptyorb");
			hitPointsFill = new Sprite("Custom/508/hitpointsfill");
			hitPointsIcon = new Sprite("Custom/508/hitpointsicon");
			emptyOrb = new Sprite("Custom/Gameframe/Orbs/emptyorb");
			backgroundFix = new Sprite("background");
			//orbFill = new Sprite("Custom/Gameframe/Orbs/Orb 2");
			hoveredEmpty = new Sprite("Custom/Gameframe/Orbs/hoveredempty");
			runIcon1 = new Sprite("Custom/Gameframe/Orbs/runicon1");
			runIcon2 = new Sprite("Custom/Gameframe/Orbs/runicon2");
			runOrb1 = new Sprite("Custom/Gameframe/Orbs/runorb1");
			runOrb2 = new Sprite("Custom/Gameframe/Orbs/runorb2");
			prayerFill = new Sprite("Custom/508/prayerfill");
			prayerIcon = new Sprite("Custom/508/prayericon");
			emptyOrbH = new Sprite("Custom/508/prayericonH");
			prayerFillH = new Sprite("Custom/508/prayerFillH");
			...
			sprite1 = new Sprite("Custom/Gameframe/sprite1");
		xpFlag = new Sprite("Custom/Gameframe/xpFlag");
			/**/
			if (!newMap) {
				chatArea = new Sprite("Custom/508/chatarea");
				tabArea = new Sprite("Custom/508/tabarea");
			...
			for(int i = 0; i <= 19; i++) {
				ORB[i] = new Sprite("Custom/Gameframe/ORB/Orb "+i+"");
			}
			for(int i = 1; i <= 15; i++) {
				ORBS[i] = new Sprite("Frame/ORBS "+i+"");
			}
			advisorH = new Sprite("Custom/Gameframe/OTHER/AdvisorH");
			advisorN = new Sprite("Custom/Gameframe/OTHER/AdvisorN");
			advisorC = new Sprite("Custom/Gameframe/OTHER/AdvisorC");
			for(int i = 0; i <= 4;i++) {
				LOGOUT[i] = new Sprite("Custom/Frame/X "+i+"");
			}
			...
			try {
				for(int j4 = 0; j4 < 8; j4++)
					...
					headIcons[idx] = new Sprite("Interfaces/Player/Prayer/Prayer " + idx);
			...
			scrollBar2 = new Sprite(streamLoader_2, "scrollbar", 1);
			  tabHover = new Sprite("tabhover");
            tabClicked = new Sprite("tabclicked");
            for(int nSI = 0; nSI <= 15; nSI++)
                newSideIcons[nSI] = new Sprite("Custom/SideIcon/icon "+nSI);
			for(int i = 0; i < 3; i++)
...
Sprite[] counter;
	...
	private void processMainScreenClick() {
		...
		if(super.clickMode3 == 1) {
			if (newMap) {
				int i = super.saveClickX - 28 - 545;// 545
		...
		for(int j2 = 0; j2 < i2; j2++) 	{
			...
			if(class9_1.type == 0) {
				...
				if(class9_1.type == 2) {
					...
					if(spellSelected == 1 && class9_1.id == spellID && spellID != 0 && sprite != null) { 
						...
						if (sprite != null)
							if(class9_1.drawsTransparent) {
								sprite.drawHerpDerpSprite(k2, l2, class9_1.trans);
							} else {
								sprite.drawSprite(k2, l2);
							}
						}
                    ...
                    if(sprite != null)
                        if(class9_1.drawsTransparent) {
							sprite.drawSprite1(k2, l2);//, alpha);
						} else {
							sprite.drawSprite(k2, l2);
						}
	...
	private void drawTooltip() {
		...
		if(itemSelected == 1 && menuActionRow < 2)
			...
			if (super.mouseY >= 15 && super.mouseY <= 321 && super.mouseX < 436)//handle middle
				drawHoverBox2(super.mouseX+10, super.mouseY, s);
			else if (super.mouseY >= 15 && super.mouseY <= 321 && super.mouseX > 436)//handle right side
				drawHoverBox2(super.mouseX-105, super.mouseY, s);
			else if (super.mouseY >= 317 && super.mouseX < 436)//handle bottom
				drawHoverBox2(super.mouseX+10, super.mouseY-35, s);
			else if (super.mouseY >= 317 && super.mouseX > 436)//handle bottom right corner
				drawHoverBox2(super.mouseX-85, super.mouseY-35, s);
			else if (super.mouseY <= 15 && super.mouseX > 436)//handle top right corner
				drawHoverBox2(super.mouseX-85, super.mouseY+15, s);
			else if (super.mouseY <= 15 && super.mouseX < 436)//handle top
				drawHoverBox2(super.mouseX+5, super.mouseY+17, s);
			}
		if(menuActionRow > 2)
	...
	public void drawSummoning() {
		if(super.mouseX >= 689 && super.mouseX <= 745 && super.mouseY >= 130 && super.mouseY < 162){
					ORB[17].drawSprite(171, 129);
					int newSum = Integer.parseInt(RSInterface.interfaceCache[4017].message);
					smallText.method382(0x00FF00, 214, "1", 155, true);
				} else {
					ORB[18].drawSprite(171, 129);
					smallText.method382(0x00FF00, 214, "1", 155, true);
				}
...
public void drawCoinParts(){
	...
	coinOrbPart.drawSprite(505+7, 93-11);
}
...
public void drawCoinOrb(){//512 nontoggle
	if(!coinToggle) {//436,87 534 x 118 y
		...
		coinOrb.drawSprite(525-300-200-20-4, 47+37);//+37
...
public int getOrbTextColor(int statusInt){
	...
	public void drawHPOrb() {
		int health;
		String OrbDirectory = signlink.findcachedir()+"/Sprites/Custom/Gameframe/ORB/";
		...
		ORB[2] = new Sprite(OrbDirectory+"Orb 2.PNG", 27, getOrbFill(health));
		ORB[0].drawSprite(173, 15);
		if(myPlayer.Poisoned == 1) {
			ORB[15].drawSprite(176, 18);
		} else {
			ORB[3].drawSprite(176, 18);
		}
		ORB[2].drawSprite(176, 18);
		if(health <= 20){
			if(loopCycle % 20 < 10){
				ORB[8].drawSprite(183, 25);
			}
		} else {
			ORB[8].drawSprite(183, 25);
		}
		int Y = 40;
		int newhp = Integer.parseInt(RSInterface.interfaceCache[4016].message);
		smallText.method382(getOrbTextColor(health), 215, ""+newhp*10, Y, true);
	}
	
	public Sprite[] ORBS = new Sprite[16];
	
	public void drawRunOrb(){
		String OD = signlink.findcachedir()+"/Sprites/Custom/Gameframe/ORB/";
		ORB[2] = new Sprite(OD+"Orb 2.PNG", 27, getOrbFill(energy+100));
		if(!runClicked){
				if(runHover){
					ORB[1].drawSprite(187, 92);
				} else{ 
					ORB[0].drawSprite(187, 92);
				}
			ORB[6].drawSprite(190, 95);
			if(!restOrb) {
				ORB[10].drawSprite(196, 100);
			} else {
				ORB[12].drawSprite(196, 100);
			}
		} else {
				if(runHover){
					ORB[1].drawSprite(187, 92);
			...
			ORB[7].drawSprite(190, 95);
			if(!restOrb) {
				ORB[11].drawSprite(196, 100);
			} else {
				ORB[12].drawSprite(196, 100);
			}
		}			
		int Y = 117;
		smallText.method382(0x00FF00, 229, "100", Y, true);
	}
	...
	public void drawPrayerOrb() {
		...
		ORB[2] = new Sprite(OD+"Orb 2.PNG", 27, getOrbFill(prayer));
		if(!prayClicked){
				if(prayHover){
					ORB[1].drawSprite(187, 53);
				}else if(!prayHover){
					ORB[0].drawSprite(187, 53);
				}	
			ORB[4].drawSprite(190, 56);
		} else if(prayClicked){
				if(prayHover){
					ORB[1].drawSprite(187, 53);
				} else if(!prayHover){
			...
			ORB[5].drawSprite(190, 56);
		}
		ORB[2].drawSprite(190, 56);
		if(prayer <= 25) {
			if(loopCycle % 20 < 10) {
				ORB[9].drawSprite(194, 60);
			}
		} else {
			ORB[9].drawSprite(194, 60);
		}
		int Y = 79;
		int newpray = Integer.parseInt(RSInterface.interfaceCache[4012].message);
		smallText.method382(getOrbTextColor(prayer), 229, ""+newpray*10, Y, true);
	}
	...
	private void drawMinimap() {
		...
		if(!newMap) {
			...
			drawSummoning();
			drawRunOrb();
			DrawingArea.drawPixels(3, 78+6, 97+20, 0xffffff, 3);
		} else
		if(newMap) {
	...
	public void drawAdv() {
				...
				if(super.mouseX > 724 && super.mouseX < 741 && super.mouseY > 1 && super.mouseY < 23) {
					advisorH.drawSprite(204, 0);
				} else {		
					advisorN.drawSprite(204, 0);
					}
				if(advisorHPos == 1) {
            advisorH.drawSprite(225, 0);
            if(tabID == 10)
                advisorC.drawSprite(225, 0);
        }
	}
	public int advisorHPos = 0;
	private Sprite advisorN;
	private Sprite advisorH;
	private Sprite advisorC;
public Sprite[] LOGOUT = new Sprite[5];
public boolean logHover = false;
	public void drawLogout() {
		LOGOUT[!logHover ? 0 : 1].drawSprite(225, 0);
		if(super.clickMode2 == 1 && super.mouseX >= 765-26 && super.mouseX <= 763 && super.mouseY >= 6 && super.mouseY <= 30){
			LOGOUT[2].drawSprite(225, 0);
			setTab(10);
		}
	}
...
//private RSImageProducer loginScreenArea;
	...
	private void markMinimap(Sprite sprite, int i, int j)
	{
		...
		try {
		if(newMap) {
			sprite.drawSprite(((125 + k1) - sprite.anInt1444 / 2) + 4, 89 - l1 - sprite.anInt1445 / 2 - 4);
		}
			} catch (Exception e) {
		}
		try {
...
//	private Sprite titleBox;
	...
	public Sprite[] combatIcons = new Sprite[7];
	/**/
